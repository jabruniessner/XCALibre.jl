...
function bounded_convection_correction!(psiEqn, mdotf, config)
    ...
    if get(ENV, "XCALIBRE_BOUNDED_DIAG", "0") == "1"
        netFlux = zeros(length(cells))
        mv = Array(mdotf.values)
        cf = Array(cell_faces); cns = Array(cell_nsign)
        for i in eachindex(cells)
            for fi in cells[i].faces_range
                netFlux[i] += mv[cf[fi]]*cns[fi]
            end
        end
        bcid = Array(boundary_cellsID)
        for i in 1:nbfaces
            netFlux[bcid[i]] += mv[i]
        end
        mx, ix = findmax(abs.(netFlux))
        open(joinpath(pwd(), "bounded_diag.csv"), "a") do io
